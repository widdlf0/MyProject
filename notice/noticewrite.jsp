...
<body>
		...
		<div class="container">
				<div class="row">
						<form method="post" action="/NoticeWriteAction.no" enctype="multipart/form-data" name="bbswrite">
						<input type="hidden" name="NoticeNick" value="<%=Nick %>">
						<!-- 여기서 hidden으로 만들어주는 이유는 닉네임값을 넘겨서 게시판 리스트에서
						누가 작성했는지 확인하기 위해서이다 -->
								<table class="table table-stried" style="text-align: center; border: 1px solid #dddddd">
										...
										<tbody>
												<tr>
														<th><input type="text" class="form-control" placeholder="글 제목" name="NoticeSubject" maxlength="50"></th>
												</tr>
												<tr>
														<td><textarea class="form-control" placeholder="글 내용" name="NoticeContent" maxlength="3000" style="height: 500px;"></textarea></td>
												</tr>
												
										</tbody>
										<tfoot>
												<tr>
														<td><input name="NoticeFile" type="file" class="btn btn-primary pull-right"/></td>
												</tr>
										</tfoot>
								</table>
<!-- 								<a class="btn btn-primary pull-right" onclick="writeform();">글쓰기</a> -->
								<input type="button" class="btn btn-primary pull-right" onclick="writeform();" value="글쓰기">
