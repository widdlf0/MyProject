...
<body style="overflow-x:hidden; overflow:scroll;">
		...
		<input type="hidden" id="numtestvalue" value="<%=noticeBean.getNoticeNum() %>">
				...
				<div class="row">
						...
						<input type="hidden" name="BbsNick" value="<%=Nick %>">
								<table class="table table-stried" style=" border: 1px solid #dddddd">
										...
										<tbody>
												<tr>
														<th class="form-control"><%=noticeBean.getNoticeSubject() %></th>
<!-- 														<th><input type="text" class="form-control" placeholder="글 제목" name="BbsSubject" maxlength="50"></th> -->
												</tr>
												<tr>
														<td class="form-control" style="height: 500px; overflow:scroll;"><%=noticeBean.getNoticeContent().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
										...
										<tfoot>
												<tr>
														<td class="form-control">
																...
																%>
																<a href="bbsupload/<%=noticeBean.getNoticeFile() %>">
																첨부파일 <%=noticeBean.getNoticeFile() %>
																</a>
																<%} %>
														</td>
<!-- 														<td><input name="BbsFile" type="file" class="btn btn-primary pull-right"/></td> -->
												...
												<tr>
														<td style="background-color: #eeeeee; text-align: center;">
																<%int num = noticeBean.getNoticeNum();
													   				String NoticeNick = Nick;
																...
																<%}else{ %>
																<a href="/NoticeModifyView.no?num=<%=noticeBean.getNoticeNum() %>" class="btn btn-primary">
																수정
																</a>&nbsp;&nbsp;
																<a href="/NoticeDeleteAction.no?num=<%=noticeBean.getNoticeNum() %>" class="btn btn-primary">
																삭제
																</a>&nbsp;&nbsp;
																<a href="/NoticeListAction.no" class="btn btn-primary">목록</a>&nbsp;&nbsp;
																<a href="/write.no" class="btn btn-primary pull-right">글쓰기</a>
																<%} %>
														</td>
												</tr>
										</tfoot>
												...
												<tr>
														<td rowspan="2" colspan="3">
															<form name="answerform">
																...
																<textarea rows="2" cols="3" style="width: 1100px;" id="textanswer" name="textanswer"></textarea>
																<%if(userID != null){ %>
