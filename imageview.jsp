...
<body style="overflow-x: hidden; overflow: scroll;">
	...
	<input type="hidden" id="numtestvalue"
		value="<%=imgBean.getImgNum()%>">
	...
	<div class="container">
		<div class="row">
			<form method="post" action="#" name="bbswrite">
				<table class="table table-stried" style="border: 1px solid #dddddd">
					<thead>
						<tr>
							<th colspan="2"
								style="background-color: #eeeeee; text-align: center;">이미지
								게시판 글 보기</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<th class="form-control"><%=imgBean.getImgSubject()%></th>
							<!-- 														<th><input type="text" class="form-control" placeholder="글 제목" name="BbsSubject" maxlength="50"></th> -->
						</tr>
						<tr>
							<td class="form-control" style="height: 600px; overflow: scroll;"><%=imgBean.getImgContent()%></td>
					...
					<tfoot>
						<tr>
								<td align="center">
										<a href="javascript:heartcount();" style="width:100px; height: 50px; text-decoration: none; font-size: 20px;">
										<img src="images/추천.png">&nbsp; ♥&nbsp;<%=imgBean.getHeartcount() %></a>
								</td>
						</tr>
						<tr>
							<td style="background-color: #eeeeee; text-align: center;">
								<%
									int num = imgBean.getImgNum();
									String imgNick = Nick;
									ImageDAO imgDAO = new ImageDAO();
									boolean usercheck = imgDAO.isImgWriter(num, imgNick);

									if (userID == null || usercheck == false) {
								%> <a
								href="javascript:history.back();"
								class="btn btn-primary pull-right">목록</a>&nbsp;&nbsp; <%
 	} else {
 %>
								<a href="/ImgModifyView.img?num=<%=imgBean.getImgNum()%>"
								class="btn btn-primary">수정 </a>&nbsp;&nbsp; <a
								href="/ImgDeleteAction.img?num=<%=imgBean.getImgNum()%>&userNick=<%=Nick%>"
								class="btn btn-primary">삭제 </a>&nbsp;&nbsp; <a
								href="javascript:history.back();" class="btn btn-primary">목록</a>&nbsp;&nbsp;
					...
					<tr>
						<td rowspan="2" colspan="3">
							<form name="answerform">
								...
								<textarea rows="2" cols="3" style="width: 89%;" id="textanswer"
									name="textanswer"></textarea>
								<%
									if (userID != null) {
								%>
								<a class="btn btn-primary pull-right"
									style="padding: 10px 15px; width: 10%;" id="textanswerbtn">댓글
									등록</a>
								<%
									} else {
								%>
								<a class="btn btn-primary pull-right"
									style="padding: 10px 15px; width: 10%;"
									href="javascript:logincheck3();">댓글 등록</a>
								<%
									}
								%>
							</form>
