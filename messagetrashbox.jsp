...
<body>
	...
	<div class="container">
		<div class="row">
			<a href="/msgNewListAction.mg" class="btn btn-primary">받은 편지함</a>
			<a href="/msgReadListAction.mg" class="btn btn-primary">읽은 편지함</a>
			<a href="/msgSendListAction.mg" class="btn btn-primary">보낸 편지함</a>
			<a href="/msgTrashBoxListAction.mg" class="btn btn-primary active">휴지통</a>
			<form>
					...
					<%
						for (int i = 0; i < msgList.size(); i++) {
							MsgBean msgBean = (MsgBean)msgList.get(i);
 							if(msgBean.getMsgAvailable() == 2 && msgBean.getMsgReceiveNick().equals(Nick)){ 
					%>
					<tr>
						<td><%=msgBean.getRnum() %></td>
						<td><a href="/msgDetailTrashBoxAction.mg?num=<%=msgBean.getMsgNum()%>"><%=msgBean.getMsgTitle() %></a></td>
				...
				<tfoot>
										<tr>
												...
												<td style="background-color: #eeeeee; text-align: center;">
														...
														[이전]&nbsp;&nbsp;&nbsp;
														<%}else{ %>
														<a href="/msgTrashBoxListAction.mg?page=<%=nowpage-1 %>">[이전]</a>&nbsp;&nbsp;&nbsp;
														<%} %>
														...
														<%for(int a=startpage; a<=endpage; a++){
																...
																<%=a %>&nbsp;&nbsp;
																<%}else{ %>
																<a href="/msgTrashBoxListAction.mg?page=<%=a %>"><%=a %></a>&nbsp;&nbsp;
																<%} %>
														<%} %>
														<%if(nowpage>=maxpage){ %>
														&nbsp;&nbsp;[다음]
														<%}else{ %>
														<a href="/msgTrashBoxListAction.mg?page=<%=nowpage+1 %>">&nbsp;&nbsp;[다음]</a>
														<%} %>
														
												</td>
												<td style="background-color: #eeeeee; text-align: center;"></td>
												<td style="background-color: #eeeeee; text-align: center;"><a href="javascript:trashboxdelete();" class="btn btn-primary pull-right">휴지통 비우기</a></td>
												
												<script>
														function trashboxdelete(){
															var check;
															check = confirm("휴지통을 비우시겠습니까?");
															
															if(check == true){
																location.href = "/msgTrashBoxAllDeleteAction.mg?msgAvailable=2";
															}else{
																alert("삭제에 실패 하였습니다.");
																location.href = "/msgTrashBoxListAction.mg";
															}
														}
												</script>
										</tr>
