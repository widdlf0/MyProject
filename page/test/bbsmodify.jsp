...
$(document).ready(function() {
	...
	nhn.husky.EZCreator.createInIFrame({
		...
		elPlaceHolder : "BbsContent", // 에디터가 그려질 textarea ID 값과 동일 해야 함.
...
function validation(){
	...
	if(contents === '<p>&nbsp;</p>' || contents === ''){ // 기본적으로 아무것도 입력하지 않아도 <p>&nbsp;</p> 값이 입력되어 있음. 
		alert("내용을 입력하세요.");
		oEditors.getById['BbsContent'].exec('FOCUS');
		return false;
	}
...
<body>
		...
		<div class="container">
				<div class="row">
						<form method="post" action="/BbsModifyAction.bo"  enctype="multipart/form-data" name="bbsModify" id="bbsModify">
						<input type="hidden" name="BbsNick" value="<%=Nick %>">
						<input type="hidden" name="BbsNum" value="<%=bbsBean.getBbsNum() %>">
								<table class="table table-stried" style="text-align: center; border: 1px solid #dddddd">
										...
										<tbody>
												<tr>
														<th><input type="text" class="form-control" placeholder="글 제목" name="BbsSubject" maxlength="50" value="<%=bbsBean.getBbsSubject() %>"></th>
												</tr>
												<tr>
														<td><textarea class="form-control" placeholder="글 내용" name="BbsContent"  id="BbsContent" maxlength="3000" style="height: 600px;">
														<%=bbsBean.getBbsContent() %>
														</textarea>
														</td>
												</tr>
										...
										<tfoot>
												<tr>
														<td>
																...
																%>
																<input name="BbsFile" type="file" class="btn btn-primary pull-right" value="<%=bbsBean.getBbsFile() %>"/>
																<%}else{ %>
																<input name="BbsFile" type="file" class="btn btn-primary pull-right" />
																<%} %>
														</td>
