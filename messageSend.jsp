...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<a href="/msgSendListAction.mg" class="btn btn-primary active">보낸 편지함</a>
			...
			<table class="table table-striped"
				...
				<thead>
					...
					<tr>
						...
						<th style="background-color: #eeeeee; text-align: center;">받은이</th>
						<th style="background-color: #eeeeee; text-align: center;">보낸날짜</th>
					</tr>
				</thead>
				<tbody>
					<%
						for (int i = 0; i < msgList.size(); i++) {
							MsgBean msgBean = (MsgBean)msgList.get(i);
 							if(msgBean.getMsgNick().equals(Nick)){ 
					%>
					<tr>
						<td><%=msgBean.getRnum() %></td>
						<td><a href="/msgDetailSendAction.mg?num=<%=msgBean.getMsgNum()%>"><%=msgBean.getMsgTitle() %></a></td>
				...
				<tfoot>
										<tr>
												...
												<td style="background-color: #eeeeee; text-align: center;">
														...
														[이전]&nbsp;&nbsp;&nbsp;
														<%}else{ %>
														<a href="/msgSendListAction.mg?page=<%=nowpage-1 %>">[이전]</a>&nbsp;&nbsp;&nbsp;
														<%} %>
														...
														<%for(int a=startpage; a<=endpage; a++){
																...
																<%=a %>&nbsp;&nbsp;
																<%}else{ %>
																<a href="/msgSendListAction.mg?page=<%=a %>"><%=a %></a>&nbsp;&nbsp;
																<%} %>
														<%} %>
														<%if(nowpage>=maxpage){ %>
														&nbsp;&nbsp;[다음]
														<%}else{ %>
														<a href="/msgSendListAction.mg?page=<%=nowpage+1 %>">&nbsp;&nbsp;[다음]</a>
														<%} %>
														<br/>
														<!-- 검색 부분 -->
														<select name="opt">
																...
																<option value="3">받은이</option>
														</select>
												<input type="text" size="20" name="condition"/>&nbsp;
												<input type="submit" value="검색"/>
												</td>
												<td style="background-color: #eeeeee; text-align: center;"></td>
												<td style="background-color: #eeeeee; text-align: center;"><a href="/messagewrite.mg" class="btn btn-primary pull-right">쪽지 보내기</a></td>
										</tr>
								</tfoot>
			</table>
			</form>
