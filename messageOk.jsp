...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<a href="/msgReadListAction.mg" class="btn btn-primary active">읽은 편지함</a>
					...
					<%
						for (int i = 0; i < msgList.size(); i++) {
							MsgBean msgBean = (MsgBean)msgList.get(i);
 							if(msgBean.getMsgAvailable() == 1 && msgBean.getMsgReceiveNick().equals(Nick)){ 
					%>
					<tr>
						<td><%=msgBean.getRnum() %></td>
						<td><a href="/msgDetailOkAction.mg?num=<%=msgBean.getMsgNum()%>"><%=msgBean.getMsgTitle() %></a></td>
				...
				<tfoot>
										<tr>
												...
												<td style="background-color: #eeeeee; text-align: center;">
														...
														[이전]&nbsp;&nbsp;&nbsp;
														<%}else{ %>
														<a href="/msgReadListAction.mg?page=<%=nowpage-1 %>">[이전]</a>&nbsp;&nbsp;&nbsp;
														<%} %>
														...
														<%for(int a=startpage; a<=endpage; a++){
																...
																<%=a %>&nbsp;&nbsp;
																<%}else{ %>
																<a href="/msgReadListAction.mg?page=<%=a %>"><%=a %></a>&nbsp;&nbsp;
																<%} %>
														<%} %>
														<%if(nowpage>=maxpage){ %>
														&nbsp;&nbsp;[다음]
														<%}else{ %>
														<a href="/msgReadListAction.mg?page=<%=nowpage+1 %>">&nbsp;&nbsp;[다음]</a>
														<%} %>
														<br/>
														<!-- 검색 부분 -->
														<select name="opt">
